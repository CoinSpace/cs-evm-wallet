...
function multiplyGasPrice(gasPrice, rbfFactor, precision = 2) {
  const factor = 10 ** precision;
  const numerator = BigInt(Math.round(rbfFactor * factor));
  const denominator = BigInt(factor);
  const value = gasPrice * numerator / denominator;
  if (value === gasPrice) return value + 1n;
  return value;
}

function sanitizeTypedData(typedData) {
  const { types, primaryType, domain, message } = typedData;
  const sanitized = { types, primaryType, domain: {}, message: {} };
  const allowedDomainFields = (types.EIP712Domain || []).map(t => t.name);
  for (const key in domain) {
    if (allowedDomainFields.includes(key)) {
      sanitized.domain[key] = domain[key];
    }
  }
  function sanitizeMessage(primaryType, data) {
    const allowed = (types[primaryType] || []).map(t => t.name);
    const result = {};
    for (const key of allowed) {
      if (data[key] !== undefined) {
        const { type } = types[primaryType].find(t => t.name === key);
        if (types[type]) {
          result[key] = sanitizeMessage(type, data[key]);
        } else {
          result[key] = data[key];
        }
      }
  ...
  sanitized.message = sanitizeMessage(primaryType, message);
  return sanitized;
}
