...
const actions = {
  transfer: EvmTransaction.ACTION_TRANSFER,
  tokenTransfer: EvmTransaction.ACTION_TOKEN_TRANSFER,
  staking: EvmTransaction.ACTION_STAKING,
  unstaking: EvmTransaction.ACTION_UNSTAKING,
  smartContractCall: EvmTransaction.ACTION_SMART_CONTRACT_CALL,
  cardDeposit: EvmTransaction.ACTION_CARD_DEPOSIT,
};
...
export default class TxTransformer {
  ...
  transformTx(tx) {
    ...
    let rbf = !incoming && tx.confirmations === 0 && actions[tx.action] !== EvmTransaction.ACTION_CARD_DEPOSIT;
    if (rbf) {
      const rbfGasPrice = utils.multiplyGasPrice((gasPrice || maxFeePerGas), this.#network.rbfFactor);
      rbf = rbfGasPrice < this.#network.maxGasPrice;
    }

    const fee = isToken
      ? new Amount(!incoming ? (gasUsed * gasPrice) : 0n, this.#wallet.platform.decimals)
