...
import { addr as Address, Transaction } from 'micro-eth-signer';
...
export default class EvmWallet extends CsWallet {
  ...
  async #getMinerFee(gasLimit) {
    if (['optimism', 'base'].includes(this.crypto.platform)) {
  ...
  async createTransaction({ gasLimit, address, amount }, seed, extraEntropy) {
    ...
    const { id, broadcast } = await this.buildTransaction({ gasLimit, address, amount }, seed, extraEntropy);
    await broadcast();
    return id;
  }

  async buildTransaction({ gasLimit, address, amount }, seed, extraEntropy) {
    super.createTransaction({ gasLimit, address, amount }, seed);
    const { value } = amount;
    address = address.toLowerCase();
    ...
    return {
      id: signedTx.hash,
      broadcast: async () => {
        await this.#api.sendTransaction(signedTx.toHex());
        if (this.crypto.type === 'coin') {
  ...
  async estimateReplacement(tx) {
    ...
    const gasPrice = utils.maxBigInt(gasParams.gasPrice || gasParams.maxFeePerGas, tx.gasPrice || tx.maxFeePerGas);
    const rbfGasPrice = utils.multiplyGasPrice(gasPrice, this.#network.rbfFactor);
  ...
  async createReplacementTransaction(tx, seed) {
    ...
    const transaction = Transaction.prepare({
      ...
      ...eip1559 ? {
        maxFeePerGas: rbfGasPrice,
        maxPriorityFeePerGas: utils.multiplyGasPrice(gasParams.maxPriorityFeePerGas, this.#network.rbfFactor),
      } : {
  ...
  async staking() {
    const { staked, apr, minStakeAmount, activationPeriod, unstakingPeriod } = await this.#api.staking(this.#address);
