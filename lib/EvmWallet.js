...
import { Amount, CsWallet, errors } from '@coinspace/cs-common';
...
export default class EvmWallet extends CsWallet {
  ...
  constructor(options = {}) {
    const network = networks[options.development ? 'testnet' : 'mainnet'][options.crypto.platform];
    if (!options.settings.bip44) options.settings.bip44 = network.bip44legacy;
  ...
  async create(seed) {
    ...
    const privateKey = utils.privateKeyFromMasterSeed(seed, this.settings.bip44);
  ...
  async estimateImport({ privateKey }) {
    ...
    const { value, fee } = await this.#prepareImport({ privateKey });
    return {
  ...
  async loadTransactions({ cursor } = {}) {
    ...
    const txTransformer = new TxTransformer({
      wallet: this,
      network: this.#network,
    });

    return {
      transactions: txTransformer.transformTxs(res.txs),
      hasMore: res.hasMore,
      cursor: res.cursor,
    };
  }
